...
describe "Scoring dice by category" do
  ...
  it "computes the dice score for Threes" do
    scorer = DiceScorer.for_category(:threes)

    expect(scorer.compute_score({d1: 1, d2: 4, d3: 4, d4: 2, d5: 6})).to eq(0)
    expect(scorer.compute_score({d1: 3, d2: 2, d3: 4, d4: 3, d5: 3})).to eq(3)
    expect(scorer.compute_score({d1: 3, d2: 3, d3: 6, d4: 3, d5: 3})).to eq(4)
  end
